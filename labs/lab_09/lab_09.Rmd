...
labs(
 title="West Virginia Approved April Loans",
    x = "Date Approved",
    y = "Loans",)
```
**A1.** 
...
labs(
 title="West Virginia Approved April Loans",
    x = "Date Approved",
    y = "Loans") +
  geom_text_repel(aes(x=date_approved, y=total_loans, label=total_loans)) +
  geom_point(aes(x=date_approved, y=total_loans, label=total_loans))
geom_point
```
...
**A2.** 
South Dakota, North Dakota, Nebraska, Georgia, Iowa, Illinois, Louisiana, Florida, Mississippi,
Montana
```{r}
All_State<- read_csv("data/state_ppp_all.csv")
...
stateppp <-get_acs(geography = "state",
              variables = c(age= "B01001_001"),
              year = 2019)

state_pop <-stateppp %>% 
  inner_join(All_State, by=c("NAME"= "state")) %>% 
...
state_pop %>% 
  ...
  coord_flip() +
  labs (
    title= "State With Most PPP Loans per 100k ",
    x = "State",
    y = "Loan Amounts",
...
**A3.**
The horizantal line represents the people who did not get their loans forgiven. The diagonal line represents everyone who got their loan fully forgiven. The middle dots represent everything in between.
```{r}
wv_subset <- data %>% 
  filter(amount < 25000) %>% 
