...
**Answer**
Has a dataframe showing the tweet, when it was sent, what their position was, what district they represent, etc. 
```{r}
...
**Answer**
The goal of the unique function is to leave words that appear a certain way, combining any duplicates. 
```{r}
...
**Answer**
This ranks the words by how often they appear or words that appear often. 
```{r}
...
**Answer**
It shows generic words that don't give any real information or context. 
...
**Answer**
Yes. Now it shows relevant words that can directly correlate to important politcal topics/discussions
```{r}
unique_words %>%
...
**Answer**
There is an overlap between certain words, as the pandemic was the primary issue impacting most Americans over the last several years. In 2021, words like vaccine become more prominent as science advanced. 
```{r}
unique_words_2020 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
After cutting it to two-word phrases, a majority of the top 10 responses were focused around the pandemic, vaccines or public health in general.
```{r}
covid_tweets %>%
  filter(created < '2021-01-01') %>%
...
**Task** Run the following code and describe the results.
**Answer**
While a lot of the phrases overlap between 2020 and 2021, 2021 exclusively focuses on the COVID 19 pandemic based on the top phrases used. 
```{r}
covid_tweets %>%
  filter(created >= '2021-01-01') %>%
  unnest_tokens(bigram, content, token = "ngrams", n = 2) %>%
...
**Task** Run the following code and describe the results.
**Answer**
Interesntingly, in 2021, both democrats and republicans used a lot of the same phrases. However, it would be interesting to also analyze the context in which it was used. 
```{r}
covid_tweets %>%
...
**Task** Run the following code and describe the results.
**Answer**
By filtering it down to republican house candidate phrases, more partisan phrases that could create divsion or discussion. 
```{r}
covid_tweets %>%
...
**Answer**
While nothing seems counter-intuitive on first glance, it is easily possible for some of these phrases to be used in a negative context despite the indivudal conotation of each word or phrase. 
```{r}
