...
**Answer**
Has a set of data that includes vaccinations by county in Maryland including categories such as county, first dose, second dose, and atleast one dose. 
...
**Answer**
Basically all the data falls within a range of a couple vaccinations. 
...
**Answer**
This data shows how many of each dose was given on a certain day while totaling those same categories until this specific point in time.
...
**Answer**
It appears that full vaccinations have decreased in recent days, possibly due to a perceived decrease in the sprad of COVID.
```{r}
...
**Answer**
Vaccinations have continued to decrease signifcantly since May/June of 2021. 
```{r fig.width=9}
ggplot() +
...
**Task** Run the following code and put the value of `sub` in the answer.
**Answer**
Same as task 12, however it is now the sub-title on the chart. 
...
**Answer**
Shows COVID called over the last 2 years, in 6 month intervals. Several counties had significant rieses into January 2022, which seems possible. 
```{r}
...
**Task** Run the following code and describe what the resulting chart shows
**Answer**
The axis labels have now been shifted, showing cases in log form rather than the previous title. Also shows a reverse trend of previous charts based on this shift. 
```{r}
...
**Answer**
The map shows that the outskirts of the state appeared to be less impacted by covid cases than the center of the state. Several counties in the middle of the state saw significantly more cases. 
```{r}
ggplot() +
