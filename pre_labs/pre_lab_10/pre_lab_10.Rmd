...
https://datawrapper.dwcdn.net/mIhnE/1/
...
Grouping the data by county and then summarizing the total number of loans total. 
```{r}
...
**Answer** Describe what the code below is doing and the output.
This filters the data to counties with more than six loans completed.
```{r}
md_counties <- maryland_ppp %>%
    group_by(project_county_name) %>%
    summarise(count=n()) %>%
...
Fips code is a combination of the county code and state code combined then mutated into a new column.
```{r}
...
Gives you the amount of loans in each county as well as the fips code for the corresponding county
```{r}
...
This shows only the areas on the map that have data assigned to it via the dataset.
...
**Answer** Describe the changes from the previous map
Rather than showing FIPS codes on the graphic, it now shows the name of the county. 
...
https://datawrapper.dwcdn.net/Bqe8G/2/