...
**Answer**
Shows state by who they voted for, by what % and then ppp numbers related to each state.
```{r}
...
**Answer**
A graph comparing a state's voting % for trump to  % of businesses receving ppe by state.
```{r}
...
**Task** Run the following code. Describe the plot. What do you see?
**Answer**
Shows a minor relationship between the two categories. Shows amount per employee compared to % for Trump.
```{r}
...
**Answer**
Compares % of Trump votes relative to business ppe % and ppe amount per employee. Smaller number means weaker connection.
```{r}
reveal_data %>%
...
**Answer**
Basically, on the scale from -1 to 1, this data states that there is a slight correlation between % for trump and the ppe that indivudals received. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**
Shows that there is a small possibility of correlation, but less than the previous question. 
```{r}
...
**Answer**
This shows, based on 2016 votes, it shows that Clinton votes received less ppp amount per employee and the businesses received less ppe than trump voters. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer**
This data is used to basically determine the probability/ correlation of a particular event. It is often used in science experiments to determine if changes are based on the experiment itself or the actual things being tested. In this case, it shows the discrepency between CLinton and Trump voters in terms of average ppe amount per employee.
```{r}
...
**Task** Run the following code. Describe the output. What do you see?
**Answer**
 In this case, it shows the discrepency between CLinton and Trump voters in terms of businesses receiving PPP loans.
```{r}
