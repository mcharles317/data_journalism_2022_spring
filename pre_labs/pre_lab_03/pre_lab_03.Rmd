...
**Answer**Double Class, Character, Double Class
```{r}
...
**Answer**:
135073, the same number as the forgiveness_amount
```{r}
ppp_maryland_loans %>%
...
**Answer**:135073, the same as the codes ran above. 
...
**Answer** No data for October 2020, It groups by month year approved. 
```{r}
ppp_maryland_loans %>%
...
**Answer**max amount is 10000000, min amount is 6 dollars. 6 dollars is odd regarding a loan because what can that small of a sum be used for?
```{r}
...
**Answer**
"Problem with `summarise()` column `total_amount`.
ℹ `total_amount = sum(amount)`.
x invalid 'type' (character) of argument"
Error in h(simpleError(msg, call)) :

It means the type is not properly inputed.
...
**Answer**
Organizes data in descending order by amount
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
...
**Answer**No misspellings remain after this adjustment.
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
