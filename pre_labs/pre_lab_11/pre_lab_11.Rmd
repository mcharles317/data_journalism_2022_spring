...
**Answer**
There are 33 columns within this data including titles such as name, address, city, state, telephone, zip code, county, population, etc. This code lists examples of code in each column and row. 
```{r}
...
**Answer**
This code appears to be showing the top  hospital categories, detailing filled in columns such as name, address, city, zip codes, etc. This includes NAICS codes, website information, and more. It even shows if hospitals have helicopter pads and if they have a trauma unit.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Breaks data down into types of hospital, summarized the amount of each and provided a geometric cue in the third column.
```{r}
md_hospitals %>%
  group_by(TYPE) %>%
  summarise(
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This output looks extremely similar to outputs loaded in previous steps. However, rather than showing 72 hospitals, it highlights on 50 general, acute care hospitals. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Now this code creates a grid, with points plotted. For example, this map is loosely similar to the shape of Maryland State. 
```{r}
md_hospitals %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code highlights the different counties, also giving insight to the various columns within the dataset.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Filters the data to match state FIPS codes. All Maryland counties have a fips code that begins with "24."
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This shows a map, with the counties outlined. However, the map is not completely accurate because of the exclusion of certain waterways. 
```{r}
md_counties %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Now the map has specific points, highlighting specific places rather than having a blank map of counties. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The map now has color coordianted dots and a key, with the purpose of highlighting the amount of beds that each hospital has. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Again, we are looking at columns such as name, address, city, zip codes, etc. This includes NAICS codes, website information, and more. It even shows if hospitals have helicopter pads and if they have a trauma unit.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code sorts by county with the most hospital beds. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code shows county population. It can be later compared to the number of beds hospitals have available. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code provides a geometry column, that can be pivotal in collecting information and trasnfering into a map. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Shows the two data sets listed below. One of which is beds per county. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Cleans the name for both counties and population columns to increase accuracy in data. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Joining data to get beds per 100k population in Maryland.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Combined the data to add a column for total beds. 
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Added a column for beds per 100k.
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
  mutate(total_beds = replace_na(total_beds,0)) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Creates a map of counties, with the focus being number of hospital beds per 100k people. 
```{r}
ggplot() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This code changed the color of the graph. 
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
...
**Answer**
Updates key to be more representative of overall data. 
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
